...
list_regex_patterns = [
    '(^|\s)(\d|[一二三四五六七八九十])[、.，,。:：>]([^\d]|$)', # 匹配类似1. 1、
    '(^|\s)\d[\)）][、.，,。:：>]?', # 匹配类似1) 2）
    '(^|\s)[a-zA-Z][、：:]'
    '(^|\s)No\.\d+', # No.1
    '(^|\s)\((\d+|[a-zA-Z]|[一二三四五六七八九十])\)|（(\d+|[a-zA-Z]|[一二三四五六七八九十])）', # (1) (一)
    '(^|\s)[①②③④⑤⑥⑦⑧⑨⑩]',
    '(^|\s)[一二三四五六七八九十]是',
    '(^|\s)其[一二三四五六七八九十]'
]

list_regex_complied_patterns = [re.compile(x) for x in list_regex_patterns]

def list_pattern(text):
    cnt = 0
    for p in list_regex_complied_patterns:
        x = p.findall(text)
        cnt += len(x)
    return cnt
...
def dup_remove(doc, sample):
    ...
    if 'most_related_para' not in doc:
        doc['most_related_para'] = find_best_question_match(doc,sample['segmented_question'])
...
def paragraph_selection(sample, mode):
    ...
    for d_idx, doc in enumerate(sample['documents']):
        ...
        for p_idx, (para_tokens, para_scores) in \
                enumerate(zip(doc['segmented_paragraphs'], doc['segmented_paragraphs_scores'])):
            # 单纯为了让步骤类内容不会被过滤
            if list_pattern(''.join(para_tokens)) > 0:
                para_scores += 0.5
            para_infos.append(
